...
public class InfluenceClient {
	...
	public Response addAllInfluencesClient(List<Map<String, String>> requestBody, String path) {
		try {
	...
	public List<Map<String, String>> fetchInfluencesClient(Object object, String path) {
		try {
			byte[] plainCredsBytes = Constants.AUTH.getBytes();
			String base64CredsBytes = Base64.getEncoder().encodeToString(plainCredsBytes);
			...
			String data = responseEntity.getBody();
			ObjectMapper mapper = new ObjectMapper();
			List<Map<String, String>> response = mapper.readValue(data, new TypeReference<List<Map<String, String>>>(){});
			return response;
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Response removeInfluencesClient(List<Map<String, String>> requestBody, String path) {
		try {
			byte[] plainCredsBytes = Constants.AUTH.getBytes();
			String base64CredsBytes = Base64.getEncoder().encodeToString(plainCredsBytes);
	...
	public Response updateInfluence(Map<String, String> requestBody, String path) {
	try {
		byte[] plainCredsBytes = Constants.AUTH.getBytes();
		String base64CredsBytes = Base64.getEncoder().encodeToString(plainCredsBytes);
